      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.xiangmutupian" label="项目图片" prop="xiangmutupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xiangmutupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="申请条件" prop="shenqingtiaojian">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="申请条件"
                  v-model="ruleForm.shenqingtiaojian" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shenqingtiaojian" label="申请条件" prop="shenqingtiaojian">
                    <span>{{ruleForm.shenqingtiaojian}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xiangmuleixing = true;
          ...
          if(o=='xiangmujine'){
            this.ruleForm.xiangmujine = obj[o];
	    this.ro.xiangmujine = true;
            continue;
          }
	...
	this.ruleForm.xiangmujieshao = this.ruleForm.xiangmujieshao.replace(reg,'../../../django70090/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zizhuxiangmu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zizhuxiangmuCrossAddOrUpdateFlag = false;
						 this.parent.search();
